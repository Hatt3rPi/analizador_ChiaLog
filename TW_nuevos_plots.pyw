...
def actualiza_registros(plots):
    with open("data/registro_plots.json", 'w') as outfile:
        json.dump(plots, outfile, indent=3)

def actualiza_directorios():
    plots_analizados={}
    descargas={}
    plots=[]
    ...
    #Actualiza los plots que cambiaron de carpeta
    for ruta in parametros.path:
        for archivo in os.listdir(ruta):
            if archivo.endswith(".plot"):
                descargas.setdefault(archivo, {"ruta":ruta})
                plots.append(archivo)
                if archivo in plots_analizados and plots_analizados[archivo]["ruta"]!=ruta:
                    print(archivo,plots_analizados[archivo]["ruta"],ruta)
                    plots_analizados[archivo]["ruta"]=ruta

    #Elimina los plots que ya no están

    for plot in plots_analizados.copy():
        if plot not in plots:
            print(plot,plots_analizados[plot]["ruta"])
            plots_analizados.pop(plot)
        #if plots_analizados[plot]["ruta"] not in parametros.path:
        #    print(plots_analizados[plot]["ruta"])

    with open("data/registro_plots.json", 'w') as outfile:
        json.dump(plots_analizados, outfile, indent=3)

def registra_nuevos_plots():
    plots=[]
    plots_por_analizar={}
    plots_analizados={}
    ...
    if len(plots)>0:
        ...
        for plot in plots:
            ...
            ####### Cambia la ruta para poder ejecutar el subprocess
                ...
                p = subprocess.run([".\chia.exe","plots","check","-n",str(parametros.check_plot_nro_proof),"-g",f"{plot}"], capture_output=True)
                proofs = bytes(parametros.check_plot_nro_proof)
                fndproof = p.stderr.decode().find("/ "+str(parametros.check_plot_nro_proof)+",")
                print(f"{plots_por_analizar[plot]['ruta']}\{plot}")
    ...
    {mensaje_check}"""                       
                    ...
                    actualiza_registros(plots_analizados)
            if info_log==False:
                if parametros.aviso_telegram==True:
...
if __name__ == '__main__':
    actualiza_directorios()
    registra_nuevos_plots()
